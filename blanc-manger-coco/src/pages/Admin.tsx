...
type BMCTab = "questions" | "answers" | "undercoverWords";
type GameTab = "bmc" | "petitbac";
...
export default function Admin() {
  ...
  useEffect(() => {
    if (!token) return;
    setLoading(true);
    Promise.all([loadQuestions(false), loadAnswers(false), loadUndercoverWords(false), loadPBCategories(false)]).finally(() => setLoading(false));
  }, [token, loadQuestions, loadAnswers, loadUndercoverWords, loadPBCategories]);
        ...
        {gameTab === "bmc" && (
          <>
            <div className="flex gap-1 mb-4 bg-white rounded-xl border border-gray-200 p-1 w-fit">
              {(["questions", "answers", "undercoverWords"] as BMCTab[]).map(t => (
                <button key={t} onClick={() => setBmcTab(t)} className={`px-4 py-2 rounded-lg text-sm font-semibold transition ${bmcTab === t ? "bg-rose-500 text-white" : "text-gray-600 hover:bg-gray-100"}`}>
                  {t === "questions" ? `Questions (${questions.length})` : t === "answers" ? `Réponses (${answers.length})` : `Mots Undercover (${undercoverWords.length})`}
                </button>
              ))}
            </div>
            ...
            {bmcTab === "undercoverWords" && (
              <div>
                <div className="flex gap-3 mb-4 items-center">
                  <input type="text" value={uSearch} onChange={e => setUSearch(e.target.value)} placeholder="Rechercher..." className="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-rose-400" />
                  ...
                  <button onClick={() => { setAddingU(true); setNewUCivilian(""); setNewUUndercover(""); }} className="bg-rose-500 hover:bg-rose-600 text-white text-sm font-semibold px-4 py-2 rounded-lg whitespace-nowrap">+ Ajouter</button>
                </div>
                {addingU && (
                  <div className="bg-white border border-rose-200 rounded-xl p-4 mb-4">
                    <div className="flex gap-3 mb-3">
                      <input type="text" value={newUCivilian} onChange={e => setNewUCivilian(e.target.value)} placeholder="Mot civil" className="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-rose-400" />
                      <input type="text" value={newUUndercover} onChange={e => setNewUUndercover(e.target.value)} placeholder="Mot undercover" className="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-rose-400" />
                    </div>
                    <div className="flex gap-2">
                      <button onClick={addUndercoverWordPair} className="bg-rose-500 hover:bg-rose-600 text-white text-sm font-semibold px-4 py-2 rounded-lg">Ajouter</button>
