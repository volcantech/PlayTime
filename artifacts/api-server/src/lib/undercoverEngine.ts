...
function pickWords(usedWordPairIds: Set<number>): [string, string, number] | null {
  ...
  if (pair.words && pair.words.length >= 2) {
    const shuffled = shuffleArray(pair.words);
    const word1 = shuffled[0];
    const word2 = shuffled[1];
    if (Math.random() > 0.5) {
      return [word1, word2, pair.id];
    } else {
      return [word2, word1, pair.id];
    }
  }
...
export function setUCVotingDuration(roomId: string, requesterId: string, duration: number): UCRoom | { error: string } {
  const room = rooms.get(roomId);
  if (!room) return { error: "Salle introuvable." };
  if (room.hostId !== requesterId) return { error: "Seul l'hôte peut modifier la durée de vote." };
  if (room.phase !== "lobby") return { error: "Impossible de modifier pendant la partie." };
  if (![10, 15, 20].includes(duration)) return { error: "Durée invalide. Choisir 10, 15 ou 20 secondes." };
  room.votingDuration = duration;
  return room;
}
...
export function markUCWordSeen(roomId: string, playerId: string): UCRoom | { error: string } {
  ...
  const allReady = connectedAlive.length > 0 && connectedAlive.every((p) => p.hasSeenWord);
  if (allReady) room.phase = "clue";
  return room;
}
...
export function submitUCClue(roomId: string, playerId: string, clue: string): UCRoom | { error: string } {
  ...
  const connectedAlive = room.players.filter((p) => p.isAlive && p.isConnected);
  if (connectedAlive.length > 0 && connectedAlive.every((p) => p.clues.length > room.clueRound)) {
...
export function startUCVoting(roomId: string): UCRoom | null {
  ...
  room.phase = "voting";
  room.discussionEndsAt = null;
  room.votingEndsAt = Date.now() + room.votingDuration * 1_000;
  for (const p of room.players) p.voteTargetId = null;
  return room;
}
...
export function voteUCPlayer(roomId: string, playerId: string, targetId: string): UCRoom | { error: string } {
  ...
  const connectedAlive = room.players.filter((p) => p.isAlive && p.isConnected);
  if (connectedAlive.length > 0 && connectedAlive.every((p) => p.voteTargetId !== null)) {
...
export function setUCPlayerDisconnected(roomId: string, playerId: string): UCRoom | null {
  ...
  if (room.phase === "reveal" && player.isAlive && !player.hasSeenWord) {
    player.hasSeenWord = true;
    const connectedAlive = room.players.filter((p) => p.isAlive && p.isConnected);
    if (connectedAlive.length > 0 && connectedAlive.every((p) => p.hasSeenWord)) {
      room.phase = "clue";
    }
  } else if (room.phase === "clue" && player.isAlive && player.clues.length <= room.clueRound) {
    const connectedAlive = room.players.filter((p) => p.isAlive && p.isConnected);
    if (connectedAlive.length > 0 && connectedAlive.every((p) => p.clues.length > room.clueRound)) {
      room.phase = "discussion";
      room.chatMessages = [];
  ...
  } else if (room.phase === "voting" && player.isAlive && !player.voteTargetId) {
    const connectedAlive = room.players.filter((p) => p.isAlive && p.isConnected);
    if (connectedAlive.length > 0 && connectedAlive.every((p) => p.voteTargetId !== null)) {
      return processVotesInternal(room);
    }
