...
export function createUCRoom(hostName: string, avatar: string, undercoverCount = 1, options?: { isPrivate?: boolean }): { room: UCRoom; playerId: string } {
...
export function submitUCClue(roomId: string, playerId: string, clue: string): UCRoom | { error: string } {
  ...
  if (room.players.filter((p) => p.isAlive).every((p) => p.clues.length > room.clueRound)) {
    ...
    room.chatMessages = [];
    room.discussionEndsAt = Date.now() + 60_000;
  }
  return room;
}
