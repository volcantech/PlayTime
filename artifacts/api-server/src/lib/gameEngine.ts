...
export function createRoom(hostName: string, avatar: string): { room: Room; playerId: string } {
...
function startRound(room: Room): void {
  ...
  if (room.questionDeck.length === 0) {
    room.questionDeck = shuffleArray(getQuestions());
  }
...
export function submitAnswer(roomId: string, playerId: string, cardId: number): Room | { error: string } {
  const room = rooms.get(roomId);
  if (!room) return { error: "Salle introuvable." };
  ...
  room.submissions.push({ playerId, cardId, card });
...
export function findPublicBMCRoom(): Room | null {
  for (const room of rooms.values()) {
    if (room.phase === "lobby" && room.players.filter(p => p.isConnected).length > 0 && room.players.length < 10) {
      return room;
    }
  }
  return null;
