...
function dealCards(room: Room, player: Player, count: number): void {
  const allAnswers = getAnswers(room.hardMode);
  const available = allAnswers.filter(c => !room.usedAnswerCardIds.has(c.id));

  const pool = available.length > 0 ? available : allAnswers;

  const notInHand = pool.filter(c => !player.hand.find(h => h.id === c.id));
  const shuffled = shuffleArray(notInHand);
