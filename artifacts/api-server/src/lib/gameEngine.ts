...
export function createRoom(hostName: string, avatar: string, options?: { isPrivate?: boolean; cardMode?: CardMode }): { room: Room; playerId: string } {
...
function dealCards(room: Room, player: Player, count: number): void {
  ...
  // Build a set of all card IDs currently held by any player
  const allInHandIds = new Set(room.players.flatMap(p => p.hand.map(c => c.id)));

  // Prefer cards not yet used and not in anyone's hand
  let available = allAnswers.filter(c => !room.usedAnswerCardIds.has(c.id) && !allInHandIds.has(c.id));

  // If exhausted, reset used pool but still avoid cards in hand
  if (available.length === 0) {
    room.usedAnswerCardIds = new Set();
    available = allAnswers.filter(c => !allInHandIds.has(c.id));
  }

  // Last resort: if every card is already in someone's hand, allow overlap (shouldn't happen in practice)
  ...
  for (let i = 0; i < count && i < shuffled.length; i++) {
    ...
    // Mark immediately so subsequent calls in the same loop don't pick the same card
...
export function pickWinner(roomId: string, requesterId: string, winnerId: string): Room | { error: string } {
  ...
  if (winnerIndex !== -1) {
    room.questionMasterIndex = winnerIndex;
  }
