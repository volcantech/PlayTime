...
export function submitBMCVote(roomId: string, playerId: string, target: { cardId?: number; targetPlayerId?: string }): Room | { error: string } {
  const room = rooms.get(roomId);
  if (!room) return { error: "Salle introuvable." };
  ...
  // Prefer cardId (the only stable identifier the client has, since playerIds are anonymized
  // during the vote phase). Keep targetPlayerId as a fallback for legacy clients.
  let targetSubmission;
  if (typeof target.cardId === "number") {
    targetSubmission = room.submissions.find(s => s.cardId === target.cardId);
  } else if (target.targetPlayerId) {
    targetSubmission = room.submissions.find(s => s.playerId === target.targetPlayerId);
  }
  if (!targetSubmission) return { error: "Réponse introuvable." };

  const targetPlayerId = targetSubmission.playerId;
