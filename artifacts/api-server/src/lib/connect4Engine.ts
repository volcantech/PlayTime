...
export function joinC4RoomAsSpectator(code: string, spectatorName: string, avatar: string): { room: C4Room; spectatorId: string } | { error: string } {
  const room = roomsByCode.get(code.toUpperCase());
  if (!room) return { error: "Salle introuvable. Vérifie le code." };
  if (room.phase === "lobby") return { error: "La partie n'a pas encore commencé. Rejoins normalement." };

  if (room.spectators.find(s => s.name.toLowerCase() === spectatorName.toLowerCase()))
    return { error: "Ce prénom est déjà utilisé par un spectateur." };

  const spectatorId = generateId();
  room.spectators.push({ id: spectatorId, name: spectatorName, avatar: avatar || "👁️" });

  return { room, spectatorId };
}

export function removeC4Spectator(roomId: string, spectatorId: string): C4Room | null {
...
export function resetC4Room(roomId: string, requesterId: string): C4Room | { error: string } {
  ...
  const winnerIndex = room.winnerId
    ? room.players.findIndex(p => p.id === room.winnerId)
    : 0;

  room.phase = "playing";
  room.board = emptyBoard();
  room.currentTurnIndex = winnerIndex >= 0 ? winnerIndex : 0;
  room.winnerId = null;
  room.isDraw = false;
