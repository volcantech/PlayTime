...
export function createC4Room(hostName: string, avatar: string): { room: C4Room; playerId: string } {
...
export function findPublicC4Room(): C4Room | null {
  for (const room of rooms.values()) {
    if (room.phase === "lobby" && room.players.filter(p => p.isConnected).length > 0 && room.players.length < 2) {
      return room;
    }
  }
  return null;
