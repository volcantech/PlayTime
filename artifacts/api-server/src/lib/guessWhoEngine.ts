...
export function startGWSelection(
  ...
  room.characters = shuffleArray(chars);
...
export function askGWQuestion(
  ...
  room.questionHistory.push({ question: q, askerId: playerId, answer: null });
  return room;
}
...
export function answerGWQuestion(
  ...
  const last = room.questionHistory[room.questionHistory.length - 1];
