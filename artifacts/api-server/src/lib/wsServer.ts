...
import {
  ...
  nextRound, nextRoundInternal, resetToLobby, setTargetScore, setHardMode, getRoom, setPlayerDisconnected,
...
import {
  ...
  setUCUndercoverCount, setUCDiscussionDuration, addUCChatMessage,
...
function scheduleUCVoting(roomId: string, msRemaining: number) {
  ...
  const timer = setTimeout(() => {
    ...
    if (!updated) return;
    broadcast(roomId, updated);
    if (updated.phase === "round-result" && updated.roundResultEndsAt) {
...
export function setupWebSocketServer(server: Server): void {
  ...
  setPBBroadcastFn((roomId, room) => {
    broadcast(roomId, room);
    if (room.phase === "scores" && room.scoresEndsAt) {
  ...
  const { type } = msg;
      ...
      if (type === "CREATE_ROOM") {
        ...
        if (gameType === "connect4") {
          ...
          const hardMode = msg.hardMode === true;
          const { room, playerId } = createRoom(name, avatar, { isPrivate, hardMode });
      ...
      if (type === "UC_VOTE") {
        ...
        if ("error" in result) { sendError(ws, result.error); return; }
        broadcast(ws.roomId, result);
        if (result.phase === "round-result" && result.roundResultEndsAt) {
      ...
      if (type === "SET_HARD_MODE") {
        const hardMode = msg.hardMode === true;
        const result = setHardMode(ws.roomId, ws.playerId, hardMode);
        if ("error" in result) { sendError(ws, result.error); return; }
        broadcast(ws.roomId, result);
        return;
      }
