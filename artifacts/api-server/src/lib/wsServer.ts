...
import { getPBCategories } from "./petitBacCategoryCache";
    ...
    ws.on("message", async (data) => {
      ...
      if (type === "PB_ADD_CATEGORY") {
        ...
        if (categoryId) {
          ...
          category = existing[0] ? { id: existing[0].id, name: existing[0].name } : null;
        } else if (categoryName) {
          const [created] = await db.insert(petitBacCategoriesTable).values({ name: categoryName, active: true }).returning();
