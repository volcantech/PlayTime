...
import {
  createGWRoom, joinGWRoom, startGWSelection, selectGWCharacter,
  ...
  getGWRoom, getGWRoomByCode, destroyGWRoom,
  type GWRoom,
} from "./guessWhoEngine";
    ...
    ws.on("close", () => {
      if (ws.roomId && ws.playerId) {
        ...
        if (ws.gameType === "connect4") {
          ...
          setGWPlayerDisconnected(ws.roomId, ws.playerId, true);
          const room = getGWRoom(ws.roomId);
          if (room) broadcast(ws.roomId, room);
        } else {
          const room = setPlayerDisconnected(ws.roomId, ws.playerId);
          if (room) broadcast(ws.roomId, room);
