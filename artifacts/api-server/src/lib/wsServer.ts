...
import {
  ...
  nextRound, nextRoundInternal, resetToLobby, setTargetScore, setCardMode, setVoteMode, submitBMCVote, getRoom, setPlayerDisconnected,
  destroyRoom, findPublicBMCRoom, removePlayerFromBMCRoom,
  type Room,
} from "./gameEngine";
import {
  createC4Room, joinC4Room, joinC4RoomAsSpectator, removeC4Spectator, dropPiece, resetC4Room,
...
function sanitizeBMCRoom(room: Room, forPlayerId: string) {
  // In vote phase, hide which card belongs to which player (anonymous until reveal)
  const isVotePhase = room.phase === "playing-vote";
  return {
    ...room,
    gameType: "bmc",
    ...
    submissions: room.phase === "playing-submit"
      ...
      : isVotePhase
        ? room.submissions.map(s => ({ playerId: "anonymous", cardId: s.cardId, card: s.card, _realPlayerId: s.playerId === forPlayerId ? s.playerId : undefined }))
        : room.submissions,
    // Only reveal who voted for what after voting is complete (round-result)
    votes: room.phase === "round-result" || room.phase === "game-over" ? room.votes : {},
    // Show own vote in vote phase
    myVote: isVotePhase ? (room.votes[forPlayerId] ?? null) : undefined,
    // In vote phase, show how many have voted (not who)
    voteCount: isVotePhase ? Object.keys(room.votes).length : undefined,
...
interface WSSpectator extends WebSocket {
  roomId?: string;
  spectatorId?: string;
  gameType?: "connect4";
  isAlive?: boolean;
...
const spectatorClients = new Map<string, Set<WSSpectator>>();

function getSpectatorClients(roomId: string): Set<WSSpectator> {
  if (!spectatorClients.has(roomId)) spectatorClients.set(roomId, new Set());
  return spectatorClients.get(roomId)!;
}
...
function broadcast(roomId: string, room: Room | C4Room | UCRoom | PBRoom | GWRoom) {
  ...
  // Also broadcast to spectators for C4 games
  if (room.gameType === "connect4") {
    const specs = getSpectatorClients(roomId);
    const payload = sanitizeC4Room(room as C4Room);
    for (const spec of specs) {
      if (spec.readyState === WebSocket.OPEN) {
        try {
          spec.send(JSON.stringify({ type: "ROOM_UPDATE", room: payload }));
        } catch (err) {
          logger.error({ err }, "Error broadcasting to spectator");
        }
      }
    }
  }
...
export function setupWebSocketServer(server: Server): void {
  ...
  wss.on("connection", (rawWs: WebSocket, req: IncomingMessage) => {
    ...
    ws.on("close", () => {
      ...
      const wsSpec = ws as unknown as WSSpectator;
      if (wsSpec.spectatorId && wsSpec.roomId) {
        const specs = getSpectatorClients(wsSpec.roomId);
        specs.delete(wsSpec);
        removeC4Spectator(wsSpec.roomId, wsSpec.spectatorId);
        const room = getC4Room(wsSpec.roomId);
        if (room) broadcast(wsSpec.roomId, room);
        return;
      }
      if (ws.roomId && ws.playerId) {
  ...
  const { type } = msg;
      ...
      if (type === "CREATE_ROOM") {
        ...
        if (gameType === "connect4") {
          ...
          const rawVoteMode = String(msg.voteMode ?? "classic");
          const voteMode = (["classic", "democratic"].includes(rawVoteMode) ? rawVoteMode : "classic") as import("./gameEngine").VoteMode;
          const { room, playerId } = createRoom(name, avatar, { isPrivate, cardMode, voteMode });
          setPlayerUserId(room, playerId, userId);
      ...
      if (type === "JOIN_ROOM") {
        ...
        if (c4Room) {
          if (c4Room.phase !== "lobby") {
            // Room is in progress — send special error with spectate hint
            ws.send(JSON.stringify({ type: "ERROR", message: "La partie a déjà commencé.", canSpectate: true, gameType: "connect4", code }));
            return;
          }
      ...
      // ── C4 SPECTATOR JOIN ────────────────────────────────────────
      if (type === "C4_JOIN_SPECTATOR") {
        const code = String(msg.code || "").trim().toUpperCase();
        const name = String(msg.name || "").trim();
        const avatar = String(msg.avatar || "👁️");
        if (!code || !name) { sendError(ws, "Code et prénom requis."); return; }

        const result = joinC4RoomAsSpectator(code, name, avatar);
        if ("error" in result) { sendError(ws, result.error); return; }

        const spec = ws as unknown as WSSpectator;
        spec.roomId = result.room.id;
        spec.spectatorId = result.spectatorId;
        spec.gameType = "connect4";
        getSpectatorClients(result.room.id).add(spec);
        ...
        ws.send(JSON.stringify({ type: "ROOM_UPDATE", room: sanitizeC4Room(result.room) }));
        return;
      }
      ...
      if (type === "KICK_PLAYER") {
        ...
        if (gameType === "undercover") {
          ...
          broadcastDestroyed(roomId, "Un joueur a été exclu. La partie est terminée.");
        } else if (gameType === "guess_who") {
          removePlayerFromGWRoom(roomId, targetPlayerId);
          broadcastDestroyed(roomId, "Un joueur a été exclu. La partie est terminée.");
        } else {
      ...
      if (type === "BMC_SET_VOTE_MODE") {
        const rawVoteMode = String(msg.voteMode ?? "classic");
        const voteMode = (["classic", "democratic"].includes(rawVoteMode) ? rawVoteMode : "classic") as import("./gameEngine").VoteMode;
        const result = setVoteMode(ws.roomId, ws.playerId, voteMode);
        if ("error" in result) { sendError(ws, result.error); return; }
        broadcast(ws.roomId, result);
        return;
      }
      ...
      if (type === "BMC_VOTE") {
        const rawCardId = (msg as { cardId?: unknown }).cardId;
        const cardId = typeof rawCardId === "number" ? rawCardId : undefined;
        const targetPlayerId = msg.targetPlayerId ? String(msg.targetPlayerId) : undefined;
        if (cardId === undefined && !targetPlayerId) { sendError(ws, "Cible du vote requise."); return; }
        const result = submitBMCVote(ws.roomId, ws.playerId, { cardId, targetPlayerId });
        if ("error" in result) { sendError(ws, result.error); return; }
        broadcast(ws.roomId, result);
