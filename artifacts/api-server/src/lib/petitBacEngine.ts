...
export function configurePBRoom(roomId: string, requesterId: string, opts: {
  ...
  if (opts.stopAtFirstDone !== undefined) room.stopAtFirstDone = opts.stopAtFirstDone;
  return room;
}
...
export function setPBDone(roomId: string, playerId: string): PBRoom | { error: string } {
  ...
  if (!player) return { error: "Joueur introuvable." };
  player.hasDone = true;
  if (room.stopAtFirstDone || room.players.every(p => p.hasDone)) {
    moveToVoting(room);
  }
  return room;
}
...
export function findPublicPBRoom(): PBRoom | null {
  for (const room of rooms.values()) {
    if (!room.isPrivate && room.phase === "lobby" && room.players.filter(p => p.isConnected).length > 0 && room.players.length < 12) {
      return room;
    }
  }
  return null;
