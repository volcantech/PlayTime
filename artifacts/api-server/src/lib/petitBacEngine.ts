...
function computeRoundScores(room: PBRoom): PBRoundScore[] {
  ...
  for (const player of room.players) {
    ...
    for (const cat of room.categories) {
      ...
      const score = sameAnswers.length === 0 ? 10 : 5;
...
export function createPBRoom(hostName: string, avatar: string, categories: PBCategory[]): { room: PBRoom; playerId: string } {
...
function startPBRound(room: PBRoom): PBRoom {
  room.currentRound++;
  room.currentLetter = pickLetter(room.excludedLetters);
...
export function setPBDone(roomId: string, playerId: string): PBRoom | { error: string } {
  ...
  if (connected.every(p => p.hasDone)) {
    moveToVoting(room);
  }
  return room;
}
