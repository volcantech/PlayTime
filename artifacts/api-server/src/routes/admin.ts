...
import { questionCardsTable, answerCardsTable, usersTable, undercoverWordPairsTable, petitBacCategoriesTable, gameSettingsTable, gameStatsTable, customAvatarsTable, guessWhoCharactersTable, gwCategoriesTable } from "@workspace/db";
...
router.get("/guess-who-characters", requireAdmin, async (req, res) => {
  ...
  if (categoryParam) conditions.push(eq(guessWhoCharactersTable.category, categoryParam));
...
router.post("/guess-who-characters", requireAdmin, async (req, res) => {
  const { name, imageUrl, category, info } = req.body;
  if (!name || typeof name !== "string" || name.trim() === "") {
    return res.status(400).json({ error: "Le nom est requis." });
  }
  ...
  const validInfo = info && typeof info === "object" && !Array.isArray(info) ? info as Record<string, string> : null;
...
router.put("/guess-who-characters/:id", requireAdmin, async (req, res) => {
  const id = Number(req.params.id);
  const { name, imageUrl, active, category, info } = req.body;
  if (!name || typeof name !== "string" || name.trim() === "") {
    return res.status(400).json({ error: "Le nom est requis." });
  }
  const validInfo = info && typeof info === "object" && !Array.isArray(info) ? info as Record<string, string> : null;
...
// ── GW CATEGORIES ────────────────────────────────────────────────────
router.get("/gw-categories", requireAdmin, async (_req, res) => {
  const categories = await db.select().from(gwCategoriesTable).orderBy(gwCategoriesTable.name);
  return res.json(categories);
});

router.post("/gw-categories", requireAdmin, async (req, res) => {
  const { name } = req.body;
  if (!name || typeof name !== "string" || name.trim() === "") {
    return res.status(400).json({ error: "Le nom est requis." });
  }
  const existing = await db.select().from(gwCategoriesTable).where(eq(gwCategoriesTable.name, name.trim()));
  if (existing.length > 0) return res.status(409).json({ error: "Cette catégorie existe déjà." });
  const [created] = await db.insert(gwCategoriesTable).values({ name: name.trim() }).returning();
  return res.status(201).json(created);
});

router.delete("/gw-categories/:id", requireAdmin, async (req, res) => {
  const id = Number(req.params.id);
  const deleted = await db.delete(gwCategoriesTable).where(eq(gwCategoriesTable.id, id)).returning();
  if (!deleted.length) return res.status(404).json({ error: "Catégorie introuvable." });
  return res.json({ success: true });
});
