...
router.post("/guess-who-characters", requireAdmin, async (req, res) => {
  const { name, emoji, traits } = req.body;
  if (!name || typeof name !== "string" || name.trim() === "") {
    return res.status(400).json({ error: "Le nom est requis." });
  }
  if (!emoji || typeof emoji !== "string" || emoji.trim() === "") {
  ...
  const validTraits: GWTraits = {
    gender: traits?.gender ?? "homme",
    hair_color: traits?.hair_color ?? "brun",
    hair_length: traits?.hair_length ?? "court",
    glasses: traits?.glasses === true,
    hat: traits?.hat === true,
    beard: traits?.beard === true,
    earrings: traits?.earrings === true,
  };
...
router.put("/guess-who-characters/:id", requireAdmin, async (req, res) => {
  const id = Number(req.params.id);
  const { name, emoji, traits, active } = req.body;
  if (!name || typeof name !== "string" || name.trim() === "") {
    return res.status(400).json({ error: "Le nom est requis." });
  }
  if (!emoji || typeof emoji !== "string" || emoji.trim() === "") {
