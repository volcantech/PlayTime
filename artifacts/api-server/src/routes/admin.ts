...
import { questionCardsTable, answerCardsTable, adminsTable, undercoverWordPairsTable, petitBacCategoriesTable, gameSettingsTable } from "@workspace/db";
...
router.get("/petitbac-categories", requireAuth, async (_req, res) => {
  const categories = await db.select().from(petitBacCategoriesTable).orderBy(petitBacCategoriesTable.id);
  return res.json(categories);
});

router.post("/petitbac-categories", requireAuth, async (req, res) => {
  const { name } = req.body;
...
router.put("/petitbac-categories/:id", requireAuth, async (req, res) => {
  const id = Number(req.params.id);
...
router.delete("/petitbac-categories/:id", requireAuth, async (req, res) => {
  const id = Number(req.params.id);
...
router.get("/game-settings", requireAuth, async (_req, res) => {
  const settings = await getGameSettings();
  return res.json(settings);
});

router.put("/game-settings/:gameKey", requireAuth, async (req, res) => {
