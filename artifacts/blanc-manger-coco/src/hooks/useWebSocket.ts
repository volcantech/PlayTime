...
export interface UCRoom {
  ...
  phase: "lobby" | "reveal" | "clue" | "voting" | "round-result" | "game-over";
  roundNumber: number;
...
export type GameType = "bmc" | "connect4" | "undercover";

export type AnyRoom = GameRoom | C4Room | UCRoom;
...
export function useWebSocket() {
  ...
  const createRoom = useCallback((name: string, avatar: string, gameType: GameType = "bmc", options?: { undercoverCount?: number }) => {
    sessionStorage.setItem("bmc_player_name", name);
    sessionStorage.setItem("bmc_player_avatar", avatar);
    send({ type: "CREATE_ROOM", name, avatar, gameType, undercoverCount: options?.undercoverCount });
  }, [send]);

  const joinRoom = useCallback((code: string, name: string, avatar: string) => {
    sessionStorage.setItem("bmc_player_name", name);
    sessionStorage.setItem("bmc_player_avatar", avatar);
    send({ type: "JOIN_ROOM", code, name, avatar });
  }, [send]);

  const quickMatch = useCallback((name: string, avatar: string, gameType: GameType = "bmc") => {
    sessionStorage.setItem("bmc_player_name", name);
    sessionStorage.setItem("bmc_player_avatar", avatar);
    send({ type: "QUICK_MATCH", name, avatar, gameType });
  }, [send]);
