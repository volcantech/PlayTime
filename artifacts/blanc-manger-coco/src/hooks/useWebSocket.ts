...
export function useWebSocket() {
  ...
  const createRoom = useCallback((name: string, avatar: string, gameType: GameType = "bmc", options?: { undercoverCount?: number; isPrivate?: boolean; cardMode?: "normal" | "adult" | "mixed" }) => {
    sessionStorage.setItem("bmc_player_name", name);
    sessionStorage.setItem("bmc_player_avatar", avatar);
    send({ type: "CREATE_ROOM", name, avatar, gameType, undercoverCount: options?.undercoverCount, isPrivate: options?.isPrivate ?? false, cardMode: options?.cardMode ?? "normal", userToken: getUserToken() });
  }, [send]);
