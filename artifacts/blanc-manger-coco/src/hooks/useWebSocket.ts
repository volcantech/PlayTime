...
export type AnyRoom = GameRoom | C4Room;
...
export function useWebSocket() {
  ...
  const createRoom = useCallback((name: string, avatar: string, gameType: "bmc" | "connect4" = "bmc") => {
    sessionStorage.setItem("bmc_player_name", name);
    sessionStorage.setItem("bmc_player_avatar", avatar);
    send({ type: "CREATE_ROOM", name, avatar, gameType });
  }, [send]);
  ...
  const quickMatch = useCallback((name: string, avatar: string, gameType: "bmc" | "connect4" = "bmc") => {
    sessionStorage.setItem("bmc_player_name", name);
    sessionStorage.setItem("bmc_player_avatar", avatar);
