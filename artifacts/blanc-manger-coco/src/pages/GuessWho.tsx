...
function CharacterAvatar({ char, size = "md" }: { char: GWCharacter; size?: "sm" | "md" | "lg" | "xl" }) {
  const sizeMap = {
    sm: { img: "w-14 h-14", emoji: "text-3xl" },
    md: { img: "w-16 h-16", emoji: "text-4xl" },
      ...
      >
        {char.imageUrl ? (
          <img
            src={char.imageUrl}
            alt={char.name}
            style={{ maxWidth: 360, maxHeight: 400, width: "100%", objectFit: "contain" }}
            className="rounded-2xl border-2 border-gray-200"
            onError={e => { (e.currentTarget as HTMLImageElement).style.display = "none"; }}
          />
        ) : (
          <span style={{ fontSize: 96, lineHeight: 1 }}>{char.emoji}</span>
        )}
        ...
        {char.info && Object.keys(char.info).length > 0 && (
          <div className="w-full bg-gray-50 rounded-xl px-4 py-3 space-y-2">
            {Object.entries(char.info).map(([key, value]) => (
...
function CharacterCard({
  ...
  const cardW = large ? "w-28" : size === "sm" ? "w-20" : "w-24";
  const cardH = large ? "h-32" : size === "sm" ? "h-24" : "h-28";
  ...
  return (
    <div className="flex flex-col items-center gap-0.5">
      <div className={`relative ${cardW} ${cardH} flex flex-col`} style={{ perspective: "600px" }}>
        ...
        >
          ...
          >
            ...
            <span className="text-xs font-semibold text-gray-700 mt-0.5 leading-tight text-center px-0.5 line-clamp-2">{char.name}</span>
            {selected && <div className="absolute top-0.5 right-0.5 w-2 h-2 rounded-full bg-rose-500" />}
          </div>
          {/* Back — shows image + name with red X overlay */}
          <div
            className="absolute inset-0 rounded-xl border-2 border-red-200 flex flex-col items-center justify-center overflow-hidden"
            style={{ backfaceVisibility: "hidden", transform: "rotateY(180deg)" }}
          >
            <CharacterAvatar char={char} size={large ? "md" : size} />
            <span className="text-xs font-semibold text-gray-500 mt-0.5 leading-tight text-center px-0.5 line-clamp-1">{char.name}</span>
            <div className="absolute inset-0 flex items-center justify-center rounded-xl" style={{ background: "rgba(239,68,68,0.18)" }}>
              <span className="text-red-600 font-black" style={{ fontSize: 44, lineHeight: 1, opacity: 0.9 }}>✕</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-0.5">
        {!isFlipped && onToggleEliminate && (
          <button
            onClick={e => { e.stopPropagation(); onToggleEliminate(); }}
            className="text-[10px] text-red-500 hover:text-red-700 border border-red-200 rounded px-1.5 py-0.5 leading-none bg-white hover:border-red-400 transition-colors font-bold"
            title="Éliminer"
          >
            ✕
          </button>
        ...
        {isFlipped && onToggleEliminate && (
          <button
            onClick={e => { e.stopPropagation(); onToggleEliminate(); }}
            className="text-[10px] text-gray-500 hover:text-gray-700 border border-gray-300 rounded px-1.5 py-0.5 leading-none bg-white hover:border-gray-500 transition-colors"
            title="Remettre"
          >
            ↩
          </button>
        ...
        {onZoom && (
          <button
            ...
            className="text-[10px] text-gray-400 hover:text-rose-600 border border-gray-200 rounded px-1.5 py-0.5 leading-none bg-white hover:border-rose-300 transition-colors"
            title="Agrandir"
          >
            🔍
          </button>
...
export function GuessWho({ room, playerId, send, error, onLeave }: Props) {
  ...
  // Auto-advance turn after answer is shown
  useEffect(() => {
    if (room.phase === "playing" && room.questionState === "answered" && room.currentAskerId === playerId) {
      const t = setTimeout(() => send({ type: "GW_NEXT_TURN" }), 1800);
      return () => clearTimeout(t);
    }
    return undefined;
  }, [room.phase, room.questionState, room.currentAskerId, playerId]);
  ...
  if (room.phase === "lobby") {
    ...
    return (
      <div className="bg-party-warm min-h-screen flex flex-col items-center justify-start px-4 py-8">
  ...
  if (room.phase === "selection") {
    ...
    return (
      <div className="bg-party-warm min-h-screen flex flex-col px-4 py-6">
        {zoomedChar && <ZoomModal char={zoomedChar} onClose={() => setZoomedChar(null)} />}
  ...
  if (room.phase === "playing") {
    ...
    return (
      <div className="bg-party-warm min-h-screen flex flex-col px-3 py-3">
        {zoomedChar && <ZoomModal char={zoomedChar} onClose={() => setZoomedChar(null)} />}

        <div className="max-w-2xl mx-auto w-full space-y-3">
          {/* Header */}
          <div className="bg-white/95 rounded-2xl px-4 py-3 flex items-center justify-between shadow gap-2">
            ...
            <div className="flex items-center gap-1 shrink-0">
              {isHost && opponent && (
                <button
                  onClick={() => send({ type: "KICK_PLAYER", targetPlayerId: opponent.id })}
                  className="text-xs font-semibold py-1 px-2 rounded-lg text-gray-400 hover:text-red-500 hover:bg-red-50 transition-colors"
                  title={`Exclure ${opponent.name}`}
                >✕</button>
              )}
              <QuitButton onLeave={onLeave} />
            </div>
                ...
                <div>
                  ...
                  {p.id === playerId && mySecretChar && (
                    <div className="flex items-center gap-1 text-xs text-rose-600">
                      ...
                      <span className="font-semibold">{mySecretChar.name}</span>
                      <button
                        onClick={() => setZoomedChar(mySecretChar)}
                        className="text-[10px] text-gray-400 hover:text-rose-600 border border-gray-200 rounded px-1 py-0.5 leading-none bg-white hover:border-rose-300 transition-colors ml-0.5"
          ...
          {/* My character info panel */}
          {mySecretChar && mySecretChar.info && Object.keys(mySecretChar.info).length > 0 && (
            <div className="bg-white/90 rounded-2xl shadow px-4 py-3">
              <div className="text-xs font-bold text-rose-600 uppercase tracking-wide mb-2">🪪 Ton personnage — {mySecretChar.name}</div>
              <div className="space-y-1.5">
                {Object.entries(mySecretChar.info).map(([k, v]) => (
                  <div key={k} className="flex flex-wrap items-baseline gap-x-1.5 text-xs">
          ...
          <div className="bg-white/90 rounded-2xl p-4 shadow">
            ...
            {room.questionState === "answered" && (
              <div>
                ...
                <div className={`text-center text-2xl font-black mb-2 ${room.currentAnswer ? "text-green-600" : "text-red-600"}`}>
                  {room.currentAnswer ? "✅ Oui !" : "❌ Non !"}
                </div>
                {canNextTurn && (
                  <div className="text-center text-gray-400 text-xs animate-pulse">Tour suivant dans un instant…</div>
                )}
                {!isMyTurn && (
            ...
            <div className="bg-white/90 rounded-2xl shadow overflow-hidden">
              ...
              {showHistory && (
                <div className="px-4 pb-3 space-y-1.5 max-h-48 overflow-y-auto">
                  {room.questionHistory.map((entry, i) => {
                    const isMine = entry.askerId === playerId;
                    return (
                      <div key={i} className={`text-xs rounded-lg px-3 py-2 ${isMine ? "bg-rose-50 border border-rose-100" : "bg-blue-50 border border-blue-100"}`}>
                        <div className="flex items-center justify-between mb-0.5">
                          <span className={`font-bold text-[10px] uppercase tracking-wide ${isMine ? "text-rose-500" : "text-blue-500"}`}>
                            {isMine ? "🙋 Toi" : `👤 ${opponent?.name ?? "Adversaire"}`}
                          </span>
                          {entry.answer !== null && (
                            <span className={`font-bold text-[11px] ${entry.answer ? "text-green-600" : "text-red-500"}`}>
                              {entry.answer ? "✅ Oui" : "❌ Non"}
                            </span>
                          )}
                        </div>
                        <span className={`${isMine ? "text-rose-800" : "text-blue-800"}`}>{i + 1}. {entry.question}</span>
                      </div>
                    );
                  })}
                </div>
  ...
  if (room.phase === "game-over") {
    ...
    return (
      <div className="bg-party-warm min-h-screen flex flex-col items-center justify-center px-4 py-10">
