...
export default function Admin({ onBack }: { onBack?: () => void }) {
  ...
  const [newGWEmoji, setNewGWEmoji] = useState("🧑");
  const [newGWTraits, setNewGWTraits] = useState({ gender: "homme", hair_color: "brun", hair_length: "court", glasses: false, hat: false, beard: false, earrings: false });
  ...
  const addGWCharacter = async () => {
    if (!token || !newGWName.trim() || !newGWEmoji.trim()) return;
    const res = await apiFetch("/guess-who-characters", token, { method: "POST", body: JSON.stringify({ name: newGWName, emoji: newGWEmoji, traits: newGWTraits }) });
    if (res.ok) { setNewGWName(""); setNewGWEmoji("🧑"); setNewGWTraits({ gender: "homme", hair_color: "brun", hair_length: "court", glasses: false, hat: false, beard: false, earrings: false }); setAddingGW(false); loadGWCharacters({ page: gwPage, search: gwSearch, active: gwActiveOnly ? "true" : "all" }); }
    else { setError(await readApiError(res)); }
  };

  const updateGWCharacter = async (char: GWCharacterItem) => {
    if (!token) return;
    const res = await apiFetch(`/guess-who-characters/${char.id}`, token, { method: "PUT", body: JSON.stringify({ name: char.name, emoji: char.emoji, traits: char.traits, active: char.active }) });
          ...
          <div>
            ...
            {addingGW && (
              <div className="bg-rose-50 border border-rose-200 rounded-xl p-4 mb-5">
                ...
                <div className="grid grid-cols-2 gap-3 mb-3">
                  <input type="text" placeholder="Nom" value={newGWName} onChange={e => setNewGWName(e.target.value)} className="border rounded-lg px-3 py-2 text-sm" />
                  <input type="text" placeholder="Emoji 🧑" value={newGWEmoji} onChange={e => setNewGWEmoji(e.target.value)} className="border rounded-lg px-3 py-2 text-sm" />
                </div>
                <div className="grid grid-cols-3 gap-3 mb-3">
                  <div>
                    <label className="text-xs text-gray-500 mb-1 block">Genre</label>
                    <select value={newGWTraits.gender} onChange={e => setNewGWTraits(t => ({ ...t, gender: e.target.value }))} className="border rounded-lg px-2 py-1.5 text-sm w-full">
                      <option value="homme">Homme</option>
                      <option value="femme">Femme</option>
                    </select>
                  </div>
                  <div>
                    <label className="text-xs text-gray-500 mb-1 block">Couleur cheveux</label>
                    <select value={newGWTraits.hair_color} onChange={e => setNewGWTraits(t => ({ ...t, hair_color: e.target.value }))} className="border rounded-lg px-2 py-1.5 text-sm w-full">
                  ...
                  <div>
                    <label className="text-xs text-gray-500 mb-1 block">Longueur cheveux</label>
                    <select value={newGWTraits.hair_length} onChange={e => setNewGWTraits(t => ({ ...t, hair_length: e.target.value }))} className="border rounded-lg px-2 py-1.5 text-sm w-full">
                ...
                <div className="flex flex-wrap gap-4 mb-4">
                  {(["glasses", "hat", "beard", "earrings"] as const).map(trait => (
                    <label key={trait} className="flex items-center gap-1.5 text-sm text-gray-700 cursor-pointer">
                      <input type="checkbox" checked={newGWTraits[trait]} onChange={e => setNewGWTraits(t => ({ ...t, [trait]: e.target.checked }))} className="accent-rose-600" />
                      {trait === "glasses" ? "Lunettes" : trait === "hat" ? "Chapeau" : trait === "beard" ? "Barbe" : "Boucles d'oreilles"}
                    </label>
                  ))}
                </div>
                <div className="flex gap-2">
                  <button onClick={addGWCharacter} className="bg-rose-600 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-rose-700">Ajouter</button>
                  <button onClick={() => setAddingGW(false)} className="px-4 py-2 rounded-lg text-sm bg-gray-100 text-gray-600 hover:bg-gray-200">Annuler</button>
                </div>
              </div>
            )}
            ...
            {editingGW && (
              <div className="bg-yellow-50 border border-yellow-300 rounded-xl p-4 mb-5">
                ...
                <div className="grid grid-cols-2 gap-3 mb-3">
                  <input type="text" value={editingGW.name} onChange={e => setEditingGW(g => g ? { ...g, name: e.target.value } : g)} className="border rounded-lg px-3 py-2 text-sm" />
                  <input type="text" value={editingGW.emoji} onChange={e => setEditingGW(g => g ? { ...g, emoji: e.target.value } : g)} className="border rounded-lg px-3 py-2 text-sm" />
                </div>
                <div className="grid grid-cols-3 gap-3 mb-3">
                  <div>
                    <label className="text-xs text-gray-500 mb-1 block">Genre</label>
                    <select value={editingGW.traits.gender} onChange={e => setEditingGW(g => g ? { ...g, traits: { ...g.traits, gender: e.target.value } } : g)} className="border rounded-lg px-2 py-1.5 text-sm w-full">
                      <option value="homme">Homme</option>
                      <option value="femme">Femme</option>
                    </select>
                  </div>
                  <div>
                    <label className="text-xs text-gray-500 mb-1 block">Couleur cheveux</label>
                    <select value={editingGW.traits.hair_color} onChange={e => setEditingGW(g => g ? { ...g, traits: { ...g.traits, hair_color: e.target.value } } : g)} className="border rounded-lg px-2 py-1.5 text-sm w-full">
                      <option value="brun">Brun</option>
                      <option value="blond">Blond</option>
                      <option value="roux">Roux</option>
                      <option value="gris">Gris</option>
                  ...
                  <div>
                    <label className="text-xs text-gray-500 mb-1 block">Longueur cheveux</label>
                    <select value={editingGW.traits.hair_length ?? "court"} onChange={e => setEditingGW(g => g ? { ...g, traits: { ...g.traits, hair_length: e.target.value } } : g)} className="border rounded-lg px-2 py-1.5 text-sm w-full">
                      <option value="court">Court</option>
                      <option value="long">Long</option>
                    </select>
                  </div>
                ...
                <div className="flex flex-wrap gap-4 mb-4">
                  {(["glasses", "hat", "beard", "earrings"] as const).map(trait => (
                    <label key={trait} className="flex items-center gap-1.5 text-sm text-gray-700 cursor-pointer">
                      <input type="checkbox" checked={editingGW.traits[trait]} onChange={e => setEditingGW(g => g ? { ...g, traits: { ...g.traits, [trait]: e.target.checked } } : g)} className="accent-rose-600" />
                      {trait === "glasses" ? "Lunettes" : trait === "hat" ? "Chapeau" : trait === "beard" ? "Barbe" : "Boucles d'oreilles"}
                    </label>
                  ))}
                </div>
                ...
                <div className="flex gap-2">
                  <button onClick={() => updateGWCharacter(editingGW)} className="bg-yellow-600 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-yellow-700">Enregistrer</button>
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 text-gray-500 text-xs uppercase tracking-wider">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left">Cheveux</th>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {gwCharacters.map(char => (
                      <tr key={char.id} className="hover:bg-gray-50 transition">
                        <td className="px-4 py-3 text-2xl">{char.emoji}</td>
                        <td className="px-4 py-3 font-semibold text-gray-800">{char.name}</td>
                        <td className="px-4 py-3 text-gray-600 capitalize">{char.traits.gender}</td>
                        <td className="px-4 py-3 text-gray-600">
                          {char.traits.hair_color !== "chauve" ? `${char.traits.hair_color}${char.traits.hair_length ? ` ${char.traits.hair_length}` : ""}` : "Chauve"}
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex gap-1 flex-wrap">
                            {char.traits.glasses && <span className="bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded text-xs">👓</span>}
                            {char.traits.hat && <span className="bg-orange-100 text-orange-700 px-1.5 py-0.5 rounded text-xs">🎩</span>}
                            {char.traits.beard && <span className="bg-gray-100 text-gray-700 px-1.5 py-0.5 rounded text-xs">🧔</span>}
                            {char.traits.earrings && <span className="bg-pink-100 text-pink-700 px-1.5 py-0.5 rounded text-xs">💎</span>}
                          </div>
                        </td>
                        <td className="px-4 py-3">
                    ...
                    {gwCharacters.length === 0 && <tr><td colSpan={7} className="text-center text-gray-400 py-8">Aucun personnage trouvé.</td></tr>}
                  </tbody>
                </table>
              </div>
            )}
