...
export default function Admin({ onBack }: { onBack?: () => void }) {
  ...
  const [newGWCategory, setNewGWCategory] = useState("");
  const [gwCategoryFilter, setGWCategoryFilter] = useState("");
  const [newGWUploading, setNewGWUploading] = useState(false);
  const [editGWUploading, setEditGWUploading] = useState(false);
  const [newGWInfo, setNewGWInfo] = useState<Record<string, string>>({});
  const [newGWInfoKey, setNewGWInfoKey] = useState("");
  const [newGWInfoVal, setNewGWInfoVal] = useState("");
  // GW Categories
  const [gwCategories, setGWCategories] = useState<GWCategoryItem[]>([]);
  const [gwCatLoading, setGWCatLoading] = useState(false);
  const [newGWCategoryName, setNewGWCategoryName] = useState("");
  const [addingGWCategory, setAddingGWCategory] = useState(false);
  ...
  const loadGWCharacters = useCallback(async (opts: { page?: number; search?: string; active?: string; category?: string } = {}) => {
    if (!token) return;
    const { page = 1, search = "", active = "true", category = "" } = opts;
    setGWLoading(true);
    try {
      const params = new URLSearchParams({ page: String(page), search });
  ...
  const loadGWCategories = useCallback(async () => {
    if (!token) return;
    setGWCatLoading(true);
    try {
      const res = await apiFetch("/gw-categories", token);
      if (res.status === 401) { logout(); return; }
      setGWCategories(await res.json());
    } catch { setError("Erreur de chargement des catégories Qui est-ce."); }
    finally { setGWCatLoading(false); }
  }, [token]);
  ...
  useEffect(() => {
    ...
    if (gameTab === "guess_who") { loadGWCharacters({ page: 1, search: "", active: "true" }); loadGWCategories(); }
  }, [gameTab, token]);
  ...
  const uploadGWImage = async (file: File, setUploading: (v: boolean) => void, setUrl: (url: string) => void) => {
    ...
    if (file.size > MAX_MB * 1024 * 1024) {
      setError(`L'image est trop lourde. Maximum ${MAX_MB} Mo.`);
      return;
    }
    setUploading(true);
    try {
  ...
  const addGWCharacter = async () => {
    ...
    const info = Object.keys(newGWInfo).length > 0 ? newGWInfo : null;
    const res = await apiFetch("/guess-who-characters", token, { method: "POST", body: JSON.stringify({ name: newGWName, imageUrl: newGWImageUrl || null, category: newGWCategory || null, info }) });
    if (res.ok) { setNewGWName(""); setNewGWImageUrl(""); setNewGWCategory(""); setNewGWInfo({}); setNewGWInfoKey(""); setNewGWInfoVal(""); setAddingGW(false); loadGWCharacters({ page: gwPage, search: gwSearch, active: gwActiveOnly ? "true" : "all", category: gwCategoryFilter }); }
    else { setError(await readApiError(res)); }
  };

  const updateGWCharacter = async (char: GWCharacterItem) => {
    if (!token) return;
    const info = char.info && Object.keys(char.info).length > 0 ? char.info : null;
    const res = await apiFetch(`/guess-who-characters/${char.id}`, token, { method: "PUT", body: JSON.stringify({ name: char.name, imageUrl: char.imageUrl || null, active: char.active, category: char.category || null, info }) });
    if (res.ok) { setEditingGW(null); loadGWCharacters({ page: gwPage, search: gwSearch, active: gwActiveOnly ? "true" : "all", category: gwCategoryFilter }); }
    else { setError(await readApiError(res)); }
  };
  ...
  const deleteGWCharacter = async (id: number) => {
    ...
    if (res.ok) loadGWCharacters({ page: gwPage, search: gwSearch, active: gwActiveOnly ? "true" : "all", category: gwCategoryFilter });
  };

  const addGWCategory = async () => {
    if (!token || !newGWCategoryName.trim()) return;
    const res = await apiFetch("/gw-categories", token, { method: "POST", body: JSON.stringify({ name: newGWCategoryName.trim() }) });
    if (res.ok) { setNewGWCategoryName(""); setAddingGWCategory(false); loadGWCategories(); }
    else { setError(await readApiError(res)); }
  };

  const deleteGWCategory = async (id: number) => {
    if (!token || !confirm("Supprimer cette catégorie ?")) return;
    const res = await apiFetch(`/gw-categories/${id}`, token, { method: "DELETE" });
    if (res.ok) loadGWCategories();
    else { setError(await readApiError(res)); }
  };
        ...
        >
          <div className="text-center mb-6">
            ...
            <h1
              ...
              style={{ backgroundImage: "linear-gradient(135deg, #fff 30%, #6366f1)" }}
            >Administration</h1>
          </div>
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              <label className="block text-xs font-bold text-white/40 mb-1.5 uppercase tracking-wider">Pseudo</label>
            ...
            <div>
              <label className="block text-xs font-bold text-white/40 mb-1.5 uppercase tracking-wider">Code d'accès</label>
            ...
            <button type="submit" disabled={loginLoading}
              className="w-full py-3.5 rounded-xl font-black text-white transition-all disabled:opacity-50"
              style={{ background: "linear-gradient(135deg, #6366f1, #4f46e5)", boxShadow: "0 6px 20px rgba(99,102,241,0.4)" }}
            >
              {loginLoading ? "Connexion…" : "Se connecter"}
            </button>
          </form>
          <p className="text-center mt-4"><a href="/" className="text-white/30 text-sm hover:text-white/60 transition-colors">← Retour au jeu</a></p>
        </div>
      </div>
    );
  }
          ...
          <div className="space-y-6">
            ...
            <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-4">
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-bold text-indigo-800 text-sm">🏷️ Catégories de thèmes</h3>
                <button
                  onClick={() => setAddingGWCategory(v => !v)}
              ...
              {addingGWCategory && (
                <div className="flex gap-2 mb-3">
                  <input
                    type="text"
                    placeholder="Ex: Rappeur, Acteur…"
                    value={newGWCategoryName}
                    onChange={e => setNewGWCategoryName(e.target.value)}
                    onKeyDown={e => e.key === "Enter" && addGWCategory()}
                  ...
                  />
                  <button onClick={addGWCategory} disabled={!newGWCategoryName.trim()} className="bg-indigo-600 text-white px-3 py-1.5 rounded-lg text-sm font-semibold disabled:opacity-50">Créer</button>
                  <button onClick={() => { setAddingGWCategory(false); setNewGWCategoryName(""); }} className="px-3 py-1.5 rounded-lg text-sm bg-gray-100 text-gray-600">Annuler</button>
                </div>
              )}
              {gwCatLoading ? (
                <span className="text-xs text-gray-400">Chargement…</span>
              ) : gwCategories.length === 0 ? (
                <span className="text-xs text-gray-400 italic">Aucune catégorie. Cliquez sur "+ Ajouter" pour en créer.</span>
              ) : (
                <div className="flex flex-wrap gap-2">
                  {gwCategories.map(cat => (
                    <div key={cat.id} className="flex items-center gap-1 bg-white border border-indigo-200 rounded-full px-3 py-1 text-xs font-semibold text-indigo-700">
                      {cat.name}
                      <button onClick={() => deleteGWCategory(cat.id)} className="ml-1 text-red-400 hover:text-red-600 font-bold">✕</button>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div>
              <div className="flex flex-wrap items-center gap-3 mb-5">
                ...
                <input
                  ...
                  onChange={e => { setGWSearch(e.target.value); setGWPage(1); loadGWCharacters({ page: 1, search: e.target.value, active: gwActiveOnly ? "true" : "all", category: gwCategoryFilter }); }}
                  className="border border-gray-200 rounded-lg px-3 py-2 text-sm w-48 focus:outline-none focus:ring-2 focus:ring-rose-300"
                />
                <select
                  value={gwCategoryFilter}
                  onChange={e => { setGWCategoryFilter(e.target.value); setGWPage(1); loadGWCharacters({ page: 1, search: gwSearch, active: gwActiveOnly ? "true" : "all", category: e.target.value }); }}
                ...
                <label className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer select-none">
                  <input type="checkbox" checked={gwActiveOnly} onChange={e => { setGWActiveOnly(e.target.checked); loadGWCharacters({ page: 1, search: gwSearch, active: e.target.checked ? "true" : "all", category: gwCategoryFilter }); }} className="accent-rose-600" />
                  Actifs seulement
                </label>
                ...
                <div className="bg-rose-50 border border-rose-200 rounded-xl p-4 mb-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <select value={newGWCategory} onChange={e => setNewGWCategory(e.target.value)} className="border rounded-lg px-3 py-2 text-sm w-full">
                        <option value="">Sans catégorie</option>
                        {gwCategories.map(cat => <option key={cat.id} value={cat.name}>{cat.name}</option>)}
                      </select>
                    </div>
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {newGWImageUrl && (
                          <>
                            ...
                            <button onClick={() => setNewGWImageUrl("")} className="text-xs text-red-400 hover:text-red-600">✕</button>
                          </>
                        )}
                      </div>
                    </div>
                  ...
                  <div>
                    <label className="text-xs text-gray-500 mb-1 block">Informations additionnelles</label>
                    <div className="space-y-1.5 mb-2">
                      {Object.entries(newGWInfo).map(([k, v]) => (
                        <div key={k} className="flex items-center gap-2 text-sm bg-white border rounded-lg px-2 py-1">
                          <span className="font-semibold text-gray-600 shrink-0">{k} :</span>
                          <span className="flex-1 text-gray-800">{v}</span>
                          <button onClick={() => setNewGWInfo(prev => { const n = { ...prev }; delete n[k]; return n; })} className="text-red-400 hover:text-red-600 text-xs font-bold">✕</button>
                        </div>
                      ))}
                    </div>
                    <div className="flex gap-1">
                      <input type="text" placeholder="Clé (ex: Statut)" value={newGWInfoKey} onChange={e => setNewGWInfoKey(e.target.value)} className="border rounded-lg px-2 py-1 text-xs w-28 focus:outline-none focus:ring-1 focus:ring-rose-300" />
                      <input type="text" placeholder="Valeur (ex: Décédé)" value={newGWInfoVal} onChange={e => setNewGWInfoVal(e.target.value)} className="border rounded-lg px-2 py-1 text-xs flex-1 focus:outline-none focus:ring-1 focus:ring-rose-300" />
                      <button
                        onClick={() => { if (newGWInfoKey.trim() && newGWInfoVal.trim()) { setNewGWInfo(prev => ({ ...prev, [newGWInfoKey.trim()]: newGWInfoVal.trim() })); setNewGWInfoKey(""); setNewGWInfoVal(""); } }}
                        disabled={!newGWInfoKey.trim() || !newGWInfoVal.trim()}
                        className="bg-gray-200 hover:bg-gray-300 text-gray-700 px-2 py-1 rounded-lg text-xs font-semibold disabled:opacity-40"
                      >+ Ajouter</button>
                    </div>
                  </div>
                  <div className="flex gap-2 mt-1">
                    ...
                    <button onClick={() => { setAddingGW(false); setNewGWName(""); setNewGWImageUrl(""); setNewGWCategory(""); setNewGWInfo({}); setNewGWInfoKey(""); setNewGWInfoVal(""); }} className="px-4 py-2 rounded-lg text-sm bg-gray-100 text-gray-600 hover:bg-gray-200">Annuler</button>
                  </div>
                </div>
              )}
              ...
              {editingGW && (
                <div className="bg-yellow-50 border border-yellow-300 rounded-xl p-4 mb-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
                    ...
                    <div>
                      <label className="text-xs text-gray-500 mb-1 block">Catégorie</label>
                      <select value={editingGW.category ?? ""} onChange={e => setEditingGW(g => g ? { ...g, category: e.target.value || null } : g)} className="border rounded-lg px-3 py-2 text-sm w-full">
                        <option value="">Sans catégorie</option>
                        {gwCategories.map(cat => <option key={cat.id} value={cat.name}>{cat.name}</option>)}
                      </select>
                    </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {editingGW.imageUrl && (
                          <>
                            ...
                            <button onClick={() => setEditingGW(g => g ? { ...g, imageUrl: null } : g)} className="text-xs text-red-400 hover:text-red-600">✕</button>
                          </>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="mb-4">
                    <label className="text-xs text-gray-500 mb-1 block">Informations additionnelles</label>
                    <div className="space-y-1.5 mb-2">
                      {Object.entries(editingGW.info ?? {}).map(([k, v]) => (
                        <div key={k} className="flex items-center gap-2 text-sm bg-white border rounded-lg px-2 py-1">
                          <span className="font-semibold text-gray-600 shrink-0">{k} :</span>
                          <input
                            type="text"
                            value={v}
                            onChange={e => setEditingGW(g => g ? { ...g, info: { ...(g.info ?? {}), [k]: e.target.value } } : g)}
                            className="flex-1 border-0 text-xs focus:outline-none text-gray-800"
                          />
                          <button onClick={() => setEditingGW(g => { if (!g) return g; const n = { ...(g.info ?? {}) }; delete n[k]; return { ...g, info: Object.keys(n).length > 0 ? n : null }; })} className="text-red-400 hover:text-red-600 text-xs font-bold shrink-0">✕</button>
                        </div>
                      ))}
                    </div>
                    <div className="flex gap-1">
                      <input id="edit-info-key" type="text" placeholder="Clé (ex: Statut)" className="border rounded-lg px-2 py-1 text-xs w-28 focus:outline-none focus:ring-1 focus:ring-yellow-300" />
                      <input id="edit-info-val" type="text" placeholder="Valeur (ex: Décédé)" className="border rounded-lg px-2 py-1 text-xs flex-1 focus:outline-none focus:ring-1 focus:ring-yellow-300" />
                      <button
                        onClick={() => {
                          const k = (document.getElementById("edit-info-key") as HTMLInputElement).value.trim();
                          const v = (document.getElementById("edit-info-val") as HTMLInputElement).value.trim();
                          if (k && v) {
                            setEditingGW(g => g ? { ...g, info: { ...(g.info ?? {}), [k]: v } } : g);
                            (document.getElementById("edit-info-key") as HTMLInputElement).value = "";
                            (document.getElementById("edit-info-val") as HTMLInputElement).value = "";
                          }
                        }}
                        className="bg-gray-200 hover:bg-gray-300 text-gray-700 px-2 py-1 rounded-lg text-xs font-semibold"
                      >+ Ajouter</button>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {gwCharacters.map(char => (
                        <tr key={char.id} className="hover:bg-gray-50 transition">
                          ...
                          <td className="px-4 py-3">
                            {char.category
                              ? <span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-indigo-100 text-indigo-700">{char.category}</span>
                      ...
                      {gwCharacters.length === 0 && <tr><td colSpan={5} className="text-center text-gray-400 py-8">Aucun personnage trouvé.</td></tr>}
                    </tbody>
                  </table>
                </div>
              )}
              ...
              {gwTotalPages > 1 && (
                <div className="flex items-center justify-center gap-2 mt-4">
                  <button disabled={gwPage <= 1} onClick={() => { const p = gwPage - 1; setGWPage(p); loadGWCharacters({ page: p, search: gwSearch, active: gwActiveOnly ? "true" : "all", category: gwCategoryFilter }); }} className="px-3 py-1 rounded border text-sm disabled:opacity-40">←</button>
                  <span className="text-sm text-gray-600">Page {gwPage} / {gwTotalPages}</span>
                  <button disabled={gwPage >= gwTotalPages} onClick={() => { const p = gwPage + 1; setGWPage(p); loadGWCharacters({ page: p, search: gwSearch, active: gwActiveOnly ? "true" : "all", category: gwCategoryFilter }); }} className="px-3 py-1 rounded border text-sm disabled:opacity-40">→</button>
                </div>
              )}
            </div>
          </div>
