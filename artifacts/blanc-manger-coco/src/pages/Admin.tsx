...
export default function Admin({ onBack }: { onBack?: () => void }) {
  ...
  const addGWCharacter = async () => {
    ...
    const res = await apiFetch("/guess-who-characters", token, { method: "POST", body: JSON.stringify({ name: newGWName, imageUrl: newGWImageUrl || null, category: newGWCategory || null }) });
    if (res.ok) { setNewGWName(""); setNewGWImageUrl(""); setNewGWCategory(""); setAddingGW(false); loadGWCharacters({ page: gwPage, search: gwSearch, active: gwActiveOnly ? "true" : "all", category: gwCategoryFilter }); }
    else { setError(await readApiError(res)); }
  };

  const updateGWCharacter = async (char: GWCharacterItem) => {
    if (!token) return;
    const res = await apiFetch(`/guess-who-characters/${char.id}`, token, { method: "PUT", body: JSON.stringify({ name: char.name, imageUrl: char.imageUrl || null, active: char.active, category: char.category || null }) });
              ...
              {addingGW && (
                <div className="bg-rose-50 border border-rose-200 rounded-xl p-4 mb-5">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => { setAddingGW(false); setNewGWName(""); setNewGWImageUrl(""); setNewGWCategory(""); }} className="px-4 py-2 rounded-lg text-sm bg-gray-100 text-gray-600 hover:bg-gray-200">Annuler</button>
                  </div>
                </div>
              )}
