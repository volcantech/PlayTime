...
type Tab = "questions" | "answers" | "undercoverWords";
...
export default function Admin() {
  ...
  const loadUndercoverWords = useCallback(async (showSpinner = true) => {
    ...
    try {
      ...
      if (res.status === 401) { logout(); return; }
      const data = await res.json();
      setUndercoverWords(data);
    } catch {
      setError("Erreur de chargement des mots Undercover.");
    } finally {
      if (showSpinner) setLoading(false);
    }
  }, [token]);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError("");
    Promise.all([loadQuestions(false), loadAnswers(false), loadUndercoverWords(false)]).finally(() => setLoading(false));
  }, [token, loadQuestions, loadAnswers, loadUndercoverWords]);
  ...
  const addUndercoverWordPair = async () => {
    ...
    const res = await apiFetch("/undercover-word-pairs", token, {
      method: "POST",
      body: JSON.stringify({ wordCivilian: newUCivilian, wordUndercover: newUUndercover }),
    });
    if (res.ok) {
      setNewUCivilian("");
      setNewUUndercover("");
  ...
  const updateUndercoverWordPair = async (pair: UndercoverWordPair) => {
    if (!token) return;
    const res = await apiFetch(`/undercover-word-pairs/${pair.id}`, token, {
      method: "PUT",
      body: JSON.stringify({
        wordCivilian: pair.wordCivilian,
        wordUndercover: pair.wordUndercover,
        active: pair.active,
      }),
    });
    if (res.ok) {
  ...
  const filteredUndercoverWords = undercoverWords.filter(pair =>
    (showInactive || pair.active) &&
    `${pair.wordCivilian} ${pair.wordUndercover}`.toLowerCase().includes(uSearch.toLowerCase())
  );
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto p-4">
        <div className="flex gap-1 mb-4 bg-white rounded-xl border border-gray-200 p-1 w-fit">
          ...
          <button
            onClick={() => setTab("undercoverWords")}
            className={`px-4 py-2 rounded-lg text-sm font-semibold transition ${tab === "undercoverWords" ? "bg-rose-500 text-white" : "text-gray-600 hover:bg-gray-100"}`}
          >
            Mots Undercover ({undercoverWords.length})
          </button>
        </div>
          ...
          <div>
            <div className="flex gap-3 mb-4 items-center">
              <input
                ...
                placeholder="Rechercher un groupe de mots..."
                className="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-rose-400"
              />
              <label className="flex items-center gap-2 text-sm text-gray-600 whitespace-nowrap">
                <input type="checkbox" checked={showInactive} onChange={e => setShowInactive(e.target.checked)} />
              ...
              <button
                onClick={() => { setAddingU(true); setNewUCivilian(""); setNewUUndercover(""); }}
                className="bg-rose-500 hover:bg-rose-600 text-white text-sm font-semibold px-4 py-2 rounded-lg whitespace-nowrap"
              >
                + Ajouter
              </button>
            ...
            {addingU && (
              <div className="bg-white border border-rose-200 rounded-xl p-4 mb-4">
                <p className="text-sm font-semibold text-gray-700 mb-2">Nouveau groupe de mots</p>
                <div className="grid gap-3 sm:grid-cols-2 mb-3">
                  <input
                    type="text"
                    value={newUCivilian}
                    onChange={e => setNewUCivilian(e.target.value)}
                    placeholder="Ex: moto"
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-rose-400"
                  />
                  <input
                    type="text"
                    value={newUUndercover}
                    onChange={e => setNewUUndercover(e.target.value)}
                    placeholder="Ex: voiture"
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-rose-400"
                  />
                </div>
                <div className="flex gap-2">
                  <button onClick={addUndercoverWordPair} className="bg-rose-500 hover:bg-rose-600 text-white text-sm font-semibold px-4 py-2 rounded-lg">
                    Ajouter
                  </button>
                  <button onClick={() => setAddingU(false)} className="text-gray-500 hover:text-gray-700 text-sm px-4 py-2 rounded-lg hover:bg-gray-100">
                    Annuler
                  </button>
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-200 bg-gray-50">
                      <th className="text-left px-4 py-3 text-gray-500 font-medium w-12">#</th>
                      <th className="text-left px-4 py-3 text-gray-500 font-medium">Mot civil</th>
                      <th className="text-left px-4 py-3 text-gray-500 font-medium">Mot Undercover</th>
                      <th className="text-left px-4 py-3 text-gray-500 font-medium w-20">Statut</th>
                      <th className="px-4 py-3 w-36"></th>
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      <tr key={pair.id} className={`border-b border-gray-100 hover:bg-gray-50 ${!pair.active ? "opacity-50" : ""}`}>
                        ...
                        <td className="px-4 py-3">
                          {editingU?.id === pair.id ? (
                            <input
                              type="text"
                              value={editingU.wordCivilian}
                              onChange={e => setEditingU({ ...editingU, wordCivilian: e.target.value })}
                              className="w-full border border-gray-300 rounded px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-rose-400"
                            />
                          ) : (
                            <span>{pair.wordCivilian}</span>
                          )}
                        </td>
                        <td className="px-4 py-3">
                          {editingU?.id === pair.id ? (
                            <input
                              type="text"
                              value={editingU.wordUndercover}
                              onChange={e => setEditingU({ ...editingU, wordUndercover: e.target.value })}
                              className="w-full border border-gray-300 rounded px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-rose-400"
                            />
                          ) : (
                        ...
                        <td className="px-4 py-3">
                          <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${pair.active ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}>
                            {pair.active ? "Actif" : "Inactif"}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex gap-1 justify-end">
                            {editingU?.id === pair.id ? (
                              <>
                                <button onClick={() => updateUndercoverWordPair(editingU)} className="bg-rose-500 text-white text-xs px-2 py-1 rounded">OK</button>
                                <button onClick={() => setEditingU(null)} className="text-gray-500 text-xs px-2 py-1 rounded hover:bg-gray-100">✕</button>
                              </>
                            ) : (
                              <>
                                <button
                                  onClick={() => setEditingU({ ...pair })}
                                  className="text-blue-500 hover:text-blue-700 text-xs px-2 py-1 rounded hover:bg-blue-50"
                                >
                                  Modifier
                                </button>
                                <button
                                  onClick={() => updateUndercoverWordPair({ ...pair, active: !pair.active })}
                                  className="text-yellow-600 hover:text-yellow-800 text-xs px-2 py-1 rounded hover:bg-yellow-50"
                                >
                                  {pair.active ? "Désact." : "Activer"}
                                </button>
                                <button
                                  onClick={() => deleteUndercoverWordPair(pair.id)}
                                  className="text-red-500 hover:text-red-700 text-xs px-2 py-1 rounded hover:bg-red-50"
                                >
                                  Suppr.
                                </button>
                    ...
                    {filteredUndercoverWords.length === 0 && (
                      <tr><td colSpan={5} className="text-center text-gray-400 py-8">Aucun groupe de mots trouvé.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
