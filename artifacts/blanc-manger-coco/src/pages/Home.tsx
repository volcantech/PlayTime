...
interface HomeProps {
  error: string | null;
  connected: boolean;
  onCreateRoom: (name: string, avatar: string, gameType: GameType, options?: { undercoverCount?: number }) => void;
...
export function Home({ error, connected, onCreateRoom, onJoinRoom, onQuickMatch, initialCode, setMyPlayerId }: HomeProps) {
  const [gameMode, setGameMode] = useState<GameType>("bmc");
  ...
  const handleCreate = () => {
    ...
    setLocalError("");
    genId();
    onCreateRoom(trimmed, avatar, gameMode, gameMode === "undercover" ? { undercoverCount } : undefined);
  };
  ...
  const rules = gameMode === "bmc" ? RULES_BMC : gameMode === "connect4" ? RULES_C4 : RULES_UNDERCOVER;
      ...
      <div className="bg-white/90 backdrop-blur rounded-2xl shadow-xl w-full max-w-sm overflow-hidden">
        ...
        <div className="p-5 space-y-4">
          ...
          {tab === "create" && gameMode === "undercover" && (
            <div>
              <label className="block text-xs font-bold text-gray-600 mb-1 uppercase tracking-wide">Nombre d'Undercover</label>
              <select
                value={undercoverCount}
                onChange={e => setUndercoverCount(Number(e.target.value))}
                className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 font-semibold focus:border-purple-400 focus:outline-none text-sm transition-colors bg-white"
              >
                {[1, 2, 3, 4, 5].map(count => (
                  <option key={count} value={count}>{count} Undercover{count > 1 ? "s" : ""}</option>
                ))}
              </select>
              <p className="text-xs text-gray-500 mt-1">Le nombre sera limité automatiquement pour garder au moins un civil.</p>
            </div>
          )}
          ...
          >
            {tab === "play"
              ? "Trouver une partie"
              : tab === "create"
                ? `Créer une salle ${gameMode === "connect4" ? "P4" : gameMode === "undercover" ? "Undercover" : "BMC"}`
                : "Rejoindre la partie"}
          </button>
