...
const RULES_GW = [
  { n: "1", text: <>Jeu en <strong>1v1</strong>. Chaque joueur choisit secrètement un personnage parmi ceux disponibles.</> },
  { n: "2", text: <>À tour de rôle, posez une <strong>question fermée</strong> (réponse oui/non) sur les traits du personnage adverse.</> },
  { n: "3", text: <>Éliminez les personnages qui ne correspondent pas aux réponses pour affiner vos suppositions.</> },
...
const GAME_MODES = [
  ...
  { id: "guess_who" as const,  icon: "🔍", label: "Qui est-ce ?",       sub: "2 joueurs"    },
];
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-10"
      ...
      <div className="text-center mb-6" style={{ transition: "all 0.3s ease" }}>
        {gameMode === "bmc" ? (
          ...
          <>
            <div className="text-6xl mb-2">🔍</div>
            <h1 className="text-5xl font-black mb-3 whitespace-nowrap"
                style={{ fontFamily: "Pacifico, cursive", color: "#0e7490", textShadow: "2px 2px 0px #cffafe" }}>
              Qui est-ce ?
            </h1>
            <p className="text-sm font-semibold text-cyan-700 bg-white/60 rounded-full px-5 py-1.5 inline-block">
              🎭 Devine le personnage secret de ton adversaire !
            </p>
          </>
        )}
      </div>
        ...
        <div className="p-5 space-y-4">
          ...
          >
            {tab === "play"
              ? "Trouver une partie"
              : tab === "create"
                ? `Créer une salle ${gameMode === "connect4" ? "P4" : gameMode === "undercover" ? "Undercover" : gameMode === "petitbac" ? "Petit Bac" : gameMode === "guess_who" ? "Qui est-ce ?" : "BMC"}`
                : "Rejoindre la partie"}
          </button>
      ...
      <div className="mt-6 bg-white/70 backdrop-blur rounded-2xl p-5 max-w-sm w-full">
        <h3 className="font-black text-rose-700 mb-3 text-center text-sm uppercase tracking-wide">
          Comment jouer ? {gameMode === "connect4" ? "🔴" : gameMode === "undercover" ? "🕵️" : gameMode === "petitbac" ? "🔤" : gameMode === "guess_who" ? "🔍" : "🃏"}
        </h3>
