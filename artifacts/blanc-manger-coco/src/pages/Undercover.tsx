          ...
          {room.phase === "voting" && (
            <>
              ...
              {isHost && (
                <button
                  onClick={skipVote}
                  className="mt-4 w-full flex items-center justify-center gap-2 rounded-2xl px-4 py-3.5 border-2 border-white/20 bg-white/10 hover:bg-white/20 text-white font-black transition-colors shadow-lg"
                >
                  <span className="text-lg leading-none">⏭️</span>
                  <span>Passer les votes</span>
                </button>
              )}
            </>
          )}

          {room.phase === "clue" && room.voteSkipped && (
            <>
              <p className="text-xs font-black text-purple-500 uppercase tracking-widest mb-2">
                Nouveau tour d'indices
              </p>
              <h2 className="text-2xl font-black text-gray-900 mb-2">
                Donnez un autre indice
              </h2>
              <p className="text-gray-500 text-sm">
                Les votes ont été passés, tout le monde peut maintenant proposer un nouvel indice.
              </p>
            </>
          )}
