  ...
  if (room.phase === "lobby") {
    return (
      <div
        ...
        style={{ background: "linear-gradient(135deg, #1a0533 0%, #2a0a4a 55%, #4a1869 100%)" }}
      >
        ...
        <div className="bg-white/10 backdrop-blur rounded-2xl p-5 w-full max-w-sm mb-4 text-center border border-white/10">
          ...
          <div className="flex items-center justify-center gap-2 mb-3">
            <div className="text-5xl font-black tracking-[0.35em] text-white">{room.code}</div>
            {room.isPrivate && (
          ...
          <p className="text-xs text-gray-400 text-center mt-2">{room.isPrivate ? "Seuls les joueurs avec ce code peuvent rejoindre" : "Partage ce lien pour inviter des amis"}</p>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-5 w-full max-w-sm mb-4 space-y-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => send({ type: "UC_SET_VOTING_DURATION", duration: Number(e.target.value) })}
                className="w-full border border-gray-300 rounded-xl px-3 py-2 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-purple-400 bg-white"
              >
          ...
          {(room.phase === "reveal" || room.phase === "clue") && (
            <section className="bg-white/10 backdrop-blur rounded-2xl p-4 border border-white/10">
              <p className="text-purple-200 text-xs font-black uppercase tracking-widest mb-3">
                {room.phase === "reveal" ? "Phase : Découverte des mots" : `Phase : Indices (${totalClueNum}/${room.players.filter(p => p.isAlive).length})`}
              </p>
            ...
            <>
              <section className="bg-white rounded-2xl shadow-xl p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <CountdownRing seconds={votingRemaining} total={room.votingDuration} color={votingRemaining <= 3 ? "#ef4444" : "#f59e0b"} />
                </div>
                <div className="grid gap-2 sm:grid-cols-2">
                  {room.players.map((player) => {
