...
export function Undercover({ room, playerId, send, error, onLeave }: Props) {
  ...
  const skipVote = () => {
    send({ type: "UC_VOTE", targetId: "__skip__" });
  };
  ...
  const myClueCount = me?.clues.length ?? 0;
  const hasSubmittedCurrentClue = myClueCount > room.clueRound;
  const canSkip = isHost && room.clueRound < 4;
          ...
          <div className="space-y-2">
            {room.players.map((player) => (
              ...
              >
                ...
                {player.id === playerId && (
                  <span className="text-xs bg-purple-400/30 text-purple-200 px-2 py-0.5 rounded-full font-bold">
                    Toi
                  </span>
                )}
              </div>
      ...
      <header className="px-4 py-3 bg-black/30 flex items-center justify-between gap-2">
        <div>
          ...
          <p className="text-purple-200 text-xs">
            ...
            {room.clueRound > 0 && ` · Indice ${room.clueRound + 1}`}
          </p>
        </div>
        <button
          onClick={onLeave}
              ...
              {!hasSubmittedCurrentClue && me?.isAlive ? (
                <div className="flex gap-2">
                  <input
                    ...
                    maxLength={40}
                    placeholder={room.clueRound === 0 ? "Ton indice en un mot ou courte phrase" : `Indice ${room.clueRound + 1}...`}
        ...
        {room.phase === "voting" && (
          <>
            {isHost && canSkip && (
              <button
                onClick={skipVote}
                className="w-full py-2 rounded-xl bg-purple-600 hover:bg-purple-500 text-white text-sm font-bold"
              >
                Passer les votes
              </button>
            )}
