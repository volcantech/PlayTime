...
export function Connect4({ room, playerId, send, error, onLeave, isSpectating = false }: Props) {
  ...
  if (room.phase === "lobby") {
    return (
      <div
        className="bg-party min-h-screen flex flex-col items-center justify-center px-4 relative overflow-hidden"
      >
  ...
  return (
    <div
      className="bg-party min-h-screen flex flex-col relative overflow-hidden"
    >
      <style>{`
        @keyframes dropFall {
