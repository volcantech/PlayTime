...
interface AnimCell { row: number; col: number; id: number }
...
export function Connect4({ room, playerId, send, error, onLeave }: Props) {
  ...
  const isMyTurn = room.phase === "playing" && room.players[room.currentTurnIndex]?.id === playerId;
      ...
      for (let r = 0; r < ROWS; r++) {
        for (let c = 0; c < COLS; c++) {
          if (!prev[r]?.[c] && room.board[r]?.[c]) {
            ...
            const timer = setTimeout(() => setAnimCell(a => a?.id === id ? null : a), 550);
  ...
  if (room.phase === "lobby") {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-4"
        ...
        <p className="text-purple-300 text-sm mb-6">En attente d'un adversaire…</p>

        <div className="bg-white/10 backdrop-blur rounded-2xl p-6 max-w-sm w-full text-center">
          <p className="text-purple-200 text-xs font-bold uppercase tracking-wide mb-2">Code de la salle</p>
          <div className="text-5xl font-black text-white tracking-widest mb-4">{room.code}</div>
                ...
                <div key={i} className="flex items-center gap-3 bg-white/10 rounded-xl px-4 py-2.5">
                  ...
                  {p ? (
                    <>
                      ...
                      <span className="text-green-400 text-xs font-bold">✓ Connecté</span>
                    </>
                  ) : (
                    <span className="text-purple-300 text-sm italic flex-1 text-left">En attente…</span>
                  )}
                </div>
              );
            })}
          ...
          <button
            onClick={onLeave}
            className="w-full py-2.5 rounded-xl bg-white/10 hover:bg-white/20 text-purple-300 font-semibold text-sm transition-colors mt-2"
          >
            🚪 Quitter
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen flex flex-col"
      ...
      <div className="px-4 py-3 bg-black/30 flex items-center justify-between gap-2">
            ...
            return (
              <div key={p.id}
                className={`flex items-center gap-1.5 rounded-full px-3 py-1 transition-all shrink-0 ${
                  isActive ? "bg-white/20 ring-2 ring-white scale-105" : "bg-white/5"
                }`}>
                <span>{c.emoji}</span>
        ...
        <button
          onClick={onLeave}
          className="shrink-0 text-xs font-semibold py-1.5 px-3 rounded-xl transition-colors"
          style={{ background: "rgba(255,255,255,0.08)", color: "#a78bfa" }}
          onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.background = "rgba(239,68,68,0.2)"; (e.currentTarget as HTMLButtonElement).style.color = "#fca5a5"; }}
          onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = "rgba(255,255,255,0.08)"; (e.currentTarget as HTMLButtonElement).style.color = "#a78bfa"; }}
        >
          🚪 Quitter
        </button>
      </div>
      ...
      <div className="px-4 py-2.5 text-center min-h-[2.5rem] flex items-center justify-center">
        {room.phase === "game-over" ? (
          room.isDraw
            ? <span className="text-yellow-300 font-black text-lg">🤝 Égalité !</span>
            : <span className="font-black text-lg text-white">
                {winner?.avatar} {winner?.name} a gagné !{" "}
                {getPlayerColor(room.players.findIndex(p => p.id === room.winnerId)).emoji}
              </span>
        ) : isMyTurn
      ...
      <div className="flex-1 flex items-center justify-center px-3 pb-4">
        <div className="w-full max-w-sm">
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    color: isMyTurn && room.phase === "playing" ? "#e2e8f0" : "transparent",
                  }}
          ...
          {/* Grid */}
          <div className="rounded-2xl p-2.5 shadow-2xl overflow-hidden"
          ...
          {room.phase === "game-over" && (
            <div className="mt-5 space-y-2">
              ...
              <button
                onClick={onLeave}
                className="w-full py-2.5 rounded-2xl font-semibold text-sm text-purple-300 bg-white/10 hover:bg-white/20 transition-colors"
              >
                🚪 Quitter la partie
              </button>
            </div>
