...
export function PetitBac({ room, playerId, send, error, onLeave }: Props) {
  ...
  const scoresRemaining = useCountdown(room.scoresEndsAt ?? null);
  ...
  if (room.phase === "lobby") {
    return (
      <div
        className="bg-party min-h-screen flex flex-col items-center justify-start px-4 py-8"
      >
            ...
            <div className="mb-4">
              <button
                onClick={() =>
                  send({
                    type: "PB_CONFIG",
                    stopAtFirstDone: !room.stopAtFirstDone,
                  })
                }
                className="w-full flex items-center gap-3 bg-white/10 hover:bg-white/20 rounded-xl px-4 py-3 transition"
              >
                <div
                  className={`w-10 h-6 rounded-full relative transition-colors ${room.stopAtFirstDone ? "bg-teal-400" : "bg-white/20"}`}
                >
                  <div
                    className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-transform ${room.stopAtFirstDone ? "translate-x-5" : "translate-x-1"}`}
                  />
                </div>
                <div className="text-left">
                  <p className="text-sm text-teal-100 font-semibold">
                    Arrêt au premier fini
                  </p>
                  <p className="text-xs text-teal-300">
                    La manche s'arrête dès qu'un joueur clique "J'ai fini"
                  </p>
                </div>
              </button>
            </div>
  ...
  if (room.phase === "playing") {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-teal-900 to-teal-700 flex flex-col">
        <header className="px-4 py-3 bg-black/20 flex items-center justify-between shrink-0">
          <div>
            <h1 className="text-white font-black text-base">🔤 Petit Bac</h1>
            <p className="text-teal-300 text-xs">Manche {room.currentRound}/{room.totalRounds} · Lettre {room.currentLetter}</p>
          </div>
          <div className="flex items-center gap-1 shrink-0">
            {isHost && room.players.filter(p => p.id !== playerId).map(p => (
              <button
                key={p.id}
                onClick={() => send({ type: "KICK_PLAYER", targetPlayerId: p.id })}
                className="text-xs font-semibold py-1 px-2 rounded-lg text-white/30 hover:text-red-400 hover:bg-red-500/10 transition-colors"
                title={`Exclure ${p.name}`}
              >✕</button>
            ))}
            <button
              onClick={onLeave}
              className="text-xs font-semibold py-1.5 px-3 rounded-xl bg-white/10 text-white/70 hover:bg-red-500/20 hover:text-red-200 transition-colors"
            >
              🚪 Quitter
            </button>
          </div>
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-indigo-900 to-purple-900 flex flex-col">
        <header className="px-4 py-3 bg-black/20 flex items-center justify-between shrink-0">
          <div>
            <h1 className="text-white font-black text-base">🔤 Petit Bac</h1>
            <p className="text-purple-300 text-xs">Vote · Manche {room.currentRound}/{room.totalRounds}</p>
          </div>
          <button
            onClick={onLeave}
  ...
  if (room.phase === "scores") {
    return (
      <div className="min-h-screen bg-gradient-to-b from-emerald-900 to-teal-900 flex flex-col">
        <header className="px-4 py-3 bg-black/20 flex items-center justify-between shrink-0">
          <div>
            <h1 className="text-white font-black text-base">🔤 Petit Bac</h1>
            <p className="text-teal-300 text-xs">Résultats · Manche {room.currentRound}/{room.totalRounds}</p>
          </div>
          <button
            onClick={onLeave}
            className="shrink-0 text-xs font-semibold py-1.5 px-3 rounded-xl bg-white/10 text-white/70 hover:bg-red-500/20 hover:text-red-200 transition-colors"
        ...
        <div className="max-w-lg mx-auto">
          ...
          {room.scoresEndsAt && (
            <div className="text-center text-teal-300 text-xs font-bold mb-3">
              ⏱️ Manche suivante automatique dans {scoresRemaining}s
            </div>
          )}
        </div>
        </div>
  ...
  if (room.phase === "game-over") {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-yellow-900 to-orange-900 flex flex-col">
        <header className="px-4 py-3 bg-black/20 flex items-center justify-between shrink-0">
          <div>
            <h1 className="text-white font-black text-base">🔤 Petit Bac</h1>
            <p className="text-orange-300 text-xs">Fin de partie !</p>
          </div>
          <button
            onClick={onLeave}
            className="shrink-0 text-xs font-semibold py-1.5 px-3 rounded-xl bg-white/10 text-white/70 hover:bg-red-500/20 hover:text-red-200 transition-colors"
